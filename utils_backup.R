    ...
    for (k in 1:length(days)) {
      ...
      for (m in 1:length(models)) {
        if (!quant) {
          ...
          # wis[k, m, ] <- compute_wis(probs = sliced_data[sliced_data$model == models[m], ]$quantile[1:7], quant = sliced_data[sliced_data$model == models[m], ]$value[1:7], y = sliced_y, average = (!quant))
          wis[k, m, ] <- compute_wis(probs = sliced_data[sliced_data$model == models[m], ]$quantile, quant = sliced_data[sliced_data$model == models[m], ]$value, y = sliced_y, average = (!quant))
        }
      }
      setTxtProgressBar(b, k)
...
compute_ensemble <- function (values, current, weights = NULL, y = NULL, y_current = NULL, method = c("mean", "median", "wis", "pinball"), lower = 0, upper = 1, quant = FALSE, hh = TRUE, models = NULL, boo_wis = TRUE, ...) {
  m <- method[1]
  ...
  if (m == "mean") {
    ...
    res <- ensemble_pinball(values = values, current = current, y = y, y_current = y_current, lower = lower, upper = upper, quant = quant, hh = hh, models = models, boo_wis = boo_wis)
  } else {
...
cost_function <- function (pars, probs, values, y, wis, q = 1, quant = FALSE, hh = TRUE, boo_wis = TRUE, ...) {
  theta <- pars[1]
  phi   <- exp(pars[2])
...
ensemble_pinball <- function (values, current, y, y_current, initial_theta = 0, probs = c(0.025, 0.100, 0.250, 0.500, 0.750, 0.900, 0.975), lower = 0, upper = 1, quant = FALSE, hh = TRUE, models = NULL, boo_wis = TRUE, name_values = TRUE, ...) {
  ...
  if (sum(wis, na.rm = TRUE) == 0) {
    ...
    if (!quant) {
      ...
      for (q in 1:length(probs)) {
        ...
        # theta[q] <- optimize(f = cost_function, lower = lower, upper = upper, maximum = FALSE, probs = probs, values = values, y = y, wis = tmp_wis, q = q, quant = quant, hh = hh, boo_wis = boo_wis)$minimum  
        est_pars <- optim(par = c(0, log(1)), f = cost_function, method = c("BFGS"), probs = probs, values = values, y = y, wis = tmp_wis, q = q, quant = quant, hh = hh, boo_wis = boo_wis)$par 
        theta[q] <- est_pars[1]
        phi[q]   <- exp(est_pars[2])
      }
    }
  }
  ...
  if (!quant) {
    ...
    result <- list(nowcast = nowcast, weights = w, theta = theta, wis = tmp_wis_list)
  }
