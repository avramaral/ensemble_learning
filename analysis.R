...
for (k in 1:length(days)) { 
  ...
  for (i in 1:length(horizon)) {
    ...
    ensemble[[as.character(dt)]][[as.character(h)]] <- compute_ensemble(values = values[[as.character(dt)]][[as.character(h)]],
                                                                        ...
                                                                        method = method,
                                                                        lower = 0, upper = 1, quant = quant, models = models)
...
w_hat <- weights_tibble(ensemble = ensemble, r = r, models = models, horizon = horizon, skip_first_days = skip_days[1], skip_last_days = skip_days[2])
plotting_weights(w_hat = w_hat, h = 0, q = probs[4]) # Choose the horizon "h" and quantile "q"
...
for (h in horizon) {
  ...
  cmb_wis[[as.character(h)]] <- compute_wis_data(data = cmb_data, truth_data = truth_data, start_date = r[1], end_date = r[2], horizon = h, models = cmb_models, skip_first_days = skip_days[1], skip_last_days = skip_days[2], quant = quant, training = FALSE) 
  if (!quant) {
...
if (quant) {
  ...
  for (q in 1:length(probs)) {
    wis_days[[q]] <- compute_wis_days(wis = cmb_wis, models = cmb_models, q = q, skip_first_days = skip_days[1], skip_last_days = skip_days[2])
    tmp_plot <- plotting_wis_days(wis_days = wis_days[[q]], q = probs[q])
    print(tmp_plot)
  }
} else {
  wis_days <- compute_wis_days(wis = cmb_wis, models = cmb_models, skip_first_days = skip_days[1], skip_last_days = skip_days[2])
